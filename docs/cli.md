...
If you find `github-desktop-plus-cli` too long to type, you can create a shorter alias in your shell (e.g. `github-plus`, or even just `github` to match the upstream CLI name).
...
```powershell
Set-Alias dp-cli github-desktop-plus-cli
```
...
```bash
alias dp-cli='github-desktop-plus-cli'
```
...
```fish
alias --save dp-cli github-desktop-plus-cli
```
