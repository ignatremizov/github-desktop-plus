...
> {
  ...
  private onTogglePinnedRepository = (repository: Repository) => {
    const familyRepositories = this.props.showWorktreesInRepoList
      ? getWorktreeFamilyRepositories(
          repository,
          this.props.repositories,
          this.props.localRepositoryStateLookup
      ...
      : [repository]
    const isPinned = familyRepositories.some(familyRepository =>
      this.state.pinnedRepositoriesIds.includes(familyRepository.id)
    )

    if (isPinned) {
      this.onUnpinRepository(repository)
    } else {
      this.onPinRepository(repository)
    }
  }
