...
describe('RepositoriesList', () => {
  it('keeps the root worktree above filtered saved linked worktree rows', () => {
    ...
    assert.deepEqual(
      ...
      [mainRepo.id, linkedRepo.id]
    )
    assert.equal(processedItems[0].item.worktree?.path, mainPath)
    assert.equal(processedItems[0].matches.title.length, 0)
    assert.equal(processedItems[0].matches.subtitle.length, 0)
    assert.equal(processedItems[1].item.worktree?.path, linkedPath)
    assert.ok(
      processedItems[1].matches.title.length > 0 ||
        processedItems[1].matches.subtitle.length > 0
    )
  })
